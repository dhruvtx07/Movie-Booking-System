...
$adminUserId = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : null; // User ID for which we want to display data (logged-in user)
...
// --- Fetching Filter Options for Dropdowns/Multiselects (Dynamically filtered for venues created by admin) ---
// Base condition for all option fetching: must be active and linked to the admin's created venues
$venue_creator_filter = " V.created_by = ? ";
$params_for_option_filters = [$adminUserId];


// All Venues created by the admin (primary filter for this dashboard)
$venue_option_where_clause = "";
$current_params_for_venues = [$adminUserId];
if (!empty($selected_events)) {
    $event_placeholders_for_venues = implode(',', array_fill(0, count($selected_events), '?'));
    $venue_option_where_clause .= " AND Ei.event_id IN ($event_placeholders_for_venues)";
    $current_params_for_venues = array_merge($current_params_for_venues, $selected_events);
}
if (!empty($selected_cities)) {
    $city_placeholders_for_venues = implode(',', array_fill(0, count($selected_cities), '?'));
    $venue_option_where_clause .= " AND C.city_id IN ($city_placeholders_for_venues)";
    $current_params_for_venues = array_merge($current_params_for_venues, $selected_cities);
}
...
$all_venues_sql = "
    ...
    LEFT JOIN event_schedule Es ON V.venue_id = Es.venue_id AND Es.is_active = 'yes'
    LEFT JOIN event_info Ei ON Es.event_id = Ei.event_id AND Ei.is_active = 'yes'
    WHERE V.is_active = 'yes'
    AND {$venue_creator_filter}
    {$venue_option_where_clause}
    ORDER BY V.venue_name, V.sub_venue_name;
";
$stmt_all_venues = $pdo->prepare($all_venues_sql);
$stmt_all_venues->execute($current_params_for_venues);
$all_venues = $stmt_all_venues->fetchAll(PDO::FETCH_ASSOC);


// Fetch cities: associated with admin's venues, and potentially filtered by selected events/venues
$city_option_where_clause = "";
$current_params_for_cities = [$adminUserId]; // Reset params for this query
if (!empty($selected_events)) {
    $event_placeholders_for_cities = implode(',', array_fill(0, count($selected_events), '?'));
    $city_option_where_clause .= " AND Ei.event_id IN ($event_placeholders_for_cities)";
    $current_params_for_cities = array_merge($current_params_for_cities, $selected_events);
}
if (!empty($selected_venues)) {
    $venue_placeholders_for_cities = implode(',', array_fill(0, count($selected_venues), '?'));
    $city_option_where_clause .= " AND V.venue_id IN ($venue_placeholders_for_cities)";
    $current_params_for_cities = array_merge($current_params_for_cities, $selected_venues);
}
...
$stmt_all_cities->execute($current_params_for_cities);
$all_cities = $stmt_all_cities->fetchAll(PDO::FETCH_ASSOC);


// Fetch events: associated with admin's venues, and potentially filtered by selected cities/venues
$event_option_where_clause = "";
$current_params_for_events = [$adminUserId]; // Reset params for this query
if (!empty($selected_cities)) {
    $city_placeholders_for_events = implode(',', array_fill(0, count($selected_cities), '?'));
    $event_option_where_clause .= " AND C.city_id IN ($city_placeholders_for_events)";
    $current_params_for_events = array_merge($current_params_for_events, $selected_cities);
}
if (!empty($selected_venues)) {
    $venue_placeholders_for_events = implode(',', array_fill(0, count($selected_venues), '?'));
    $event_option_where_clause .= " AND V.venue_id IN ($venue_placeholders_for_events)";
    $current_params_for_events = array_merge($current_params_for_events, $selected_venues);
}
...
$all_events_sql = "
    ...
    LEFT JOIN cities C ON V.city_id = C.city_id AND C.is_active = 'yes'
...
$stmt_all_events->execute($current_params_for_events);
$all_events = $stmt_all_events->fetchAll(PDO::FETCH_ASSOC);


// Fetch ticket types: associated with admin's venues, and potentially filtered by selected events/cities/venues
$ticket_type_option_where_clause = "";
$current_params_for_ticket_types = [$adminUserId]; // Reset params for this query
if (!empty($selected_events)) {
    $event_placeholders_for_tickets = implode(',', array_fill(0, count($selected_events), '?'));
    $ticket_type_option_where_clause .= " AND Ei.event_id IN ($event_placeholders_for_tickets)";
    $current_params_for_ticket_types = array_merge($current_params_for_ticket_types, $selected_events);
}
if (!empty($selected_cities)) {
    $city_placeholders_for_tickets = implode(',', array_fill(0, count($selected_cities), '?'));
    $ticket_type_option_where_clause .= " AND C.city_id IN ($city_placeholders_for_tickets)";
    $current_params_for_ticket_types = array_merge($current_params_for_ticket_types, $selected_cities);
}
if (!empty($selected_venues)) {
    $venue_placeholders_for_tickets = implode(',', array_fill(0, count($selected_venues), '?'));
    $ticket_type_option_where_clause .= " AND V.venue_id IN ($venue_placeholders_for_tickets)";
    $current_params_for_ticket_types = array_merge($current_params_for_ticket_types, $selected_venues);
}
...
$all_ticket_types_sql = "
    ...
    LEFT JOIN event_info Ei ON es.event_id = Ei.event_id AND Ei.is_active = 'yes'
    LEFT JOIN cities C ON V.city_id = C.city_id AND C.is_active = 'yes'
    WHERE {$venue_creator_filter}
    {$ticket_type_option_where_clause}
    ORDER BY est.ticket_type;
";
$stmt_all_ticket_types = $pdo->prepare($all_ticket_types_sql);
$stmt_all_ticket_types->execute($current_params_for_ticket_types);
$all_ticket_types = $stmt_all_ticket_types->fetchAll(PDO::FETCH_ASSOC);


// --- Build Dynamic SQL Queries - Base Components ---

// Initial parameters common to all queries (date range + admin user ID for venue creator filter)
$base_params = [$start_datetime, $end_datetime, $adminUserId];
// Now filtering by venues.created_by for this dashboard
$main_where_clauses = ["b.booked_at BETWEEN ? AND ?", "v.created_by = ?"];

// This array explicitly tracks aliases needed for the main WHERE clause filters.
// 'v' (venues) is always needed due to the global filter 'v.created_by = ?'
...
if (!empty($selected_events)) {
    ...
    $core_query_aliases[] = 'ei'; // Need 'ei' alias if event_id is filtered
}
if (!empty($selected_cities)) {
...
if (!empty($selected_venues)) {
    ...
    // 'v' is already in core_query_aliases
}
...
// 'v' is included here because 'v.created_by = ?' is a global filter for this dashboard.
...
// The EXISTS clause ensures that only revenue_info linked to bookings related to venues created by the admin user
...
$host_commission_collected = 0; // Host fees might be collected by platform
$creator_revenue_amount = 0; // This is the amount paid to the venue creator (logged-in user)
foreach ($commissions as $comm) {
    if ($comm['commission_name'] === 'Platform Fee') {
        $platform_commission_collected = $comm['total_commission_amount'] ?? 0;
    } elseif ($comm['commission_name'] === 'Host Fee') { // This would be the fee collected by platform from host
        $host_commission_collected = $comm['total_commission_amount'] ?? 0;
    } elseif ($comm['commission_name'] === 'Creator Revenue') { // This is the payout to the creator
...
// 5. Top 5 Events by Revenue (for admin's venues)
$sql = "
    SELECT
        ei.event_name,
        ei.event_category,
...
// 6. Top 5 Cities by Revenue (for admin's venues)
$sql = "
    SELECT
        c.city_name,
...
// 7. Top 5 Venues by Revenue (admin's venues)
// This query is now based on 'v.created_by' as the primary filter.
$sql = "
    SELECT
...
<div class="content-wrapper" id="contentWrapper">
    ...
    <main class="main-content" id="mainContent">
        <div class="container py-4">
            <h2 class="section-title">My Venues Analytics</h2>
                ...
                <form action="" method="GET" id="filterForm">
                    <div class="row g-3">
                        ...
                        <div class="col-md-6 col-lg-3 filter-group-spacing">
                            ...
                            <div class="filter-checkbox-group" id="cities-group">
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label" for="all_cities">All Cities with My Venues</label>
                                </div>
                        ...
                        <div class="col-md-6 col-lg-3 filter-group-spacing">
                            ...
                            <div class="filter-checkbox-group" id="events-group">
                                <div class="form-check">
                                    ...
                                    <label class="form-check-label" for="all_events">All Events in My Venues</label>
                                </div>
                        ...
                        <div class="col-12 text-end">
                            ...
                            <a href="venue_dashboard.php" class="btn btn-secondary ms-2"><i class="fas fa-sync-alt me-2"></i>Reset Filters</a>
                        </div>
                    </div>
                </form>
            </div>

            <h3 class="section-subtitle">Overall Performance (My Venues)</h3>
            <div class="row mb-4">
                <div class="col-md-12 mb-3"> <!-- First sub-row for tickets and breakdown -->
                    <div class="row">
                        <div class="col-lg-8 mb-3"> <!-- Tickets Sold (wide) -->
                            <div class="metric-card">
                                <h5>Tickets Sold for My Venues</h5>
                                <div class="display-4"><i class="fas fa-ticket-alt"></i> <?= number_format($tickets_sold_count) ?></div>
                                <p class="text-muted">Total number of tickets booked for events in your venues between<br><?= (new DateTime($start_datetime))->format('M d, Y') ?> - <?= (new DateTime($end_datetime))->format('M d, Y') ?></p>
                            </div>
                        </div>
                            ...
                            <div class="metric-card">
                                ...
                                <ul class="list-group list-group-flush">
                                    <?php if (empty($ticket_breakdown)): ?>
                                        <li class="list-group-item">No tickets sold for events in your venues for the selected filters.</li>
                                    <?php else: ?>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            <div class="metric-card blue">
                                ...
                                <p class="text-muted">Sum of original ticket prices for events in your venues before any discounts.</p>
                            </div>
                        </div>
                        <div class="col-md-4 mb-3">
                            <div class="metric-card red">
                                ...
                                <p class="text-muted">Total value of promo codes and other discounts applied to events in your venues.</p>
                            </div>
                        </div>
                        <div class="col-md-4 mb-3">
                            <div class="metric-card green">
                                ...
                                <p class="text-muted">Total amount paid by customers for events in your venues after all discounts.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <h3 class="section-subtitle">Revenue Breakdown (My Venues)</h3>
            <div class="row mb-4">
                <div class="col-md-12 mb-3">
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            <div class="metric-card purple">
                                <h5>Platform Charges Applied</h5>
                                <div class="display-4"><i class="fas fa-city"></i> <?= number_format($platform_commission_collected, 2) ?></div>
                                <p class="text-muted">Charges applied by the platform on events in your venues.</p>
                            </div>
                        </div>
                        <div class="col-md-4 mb-3">
                            <div class="metric-card cyan">
                                <h5>Other Host Charges Collected</h5>
                                <div class="display-4"><i class="fas fa-user-tie"></i> <?= number_format($host_commission_collected, 2) ?></div>
                                <p class="text-muted">Any other Host specific charges collected on events in your venues (e.g. host fee).</p>
                            </div>
                        </div>
                        <div class="col-md-4 mb-3">
                            <div class="metric-card gold">
                                <h5>Your Revenue from Venues</h5>
                                <div class="display-4"><i class="fas fa-indian-rupee-sign"></i> <?= number_format($creator_revenue_amount, 2) ?></div>
                                <p class="text-muted">The portion of net revenue allocated directly to you as venue owner from sales in your venues.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <h3 class="section-subtitle">Promo Code Performance (My Venues)</h3>
            <?php if (empty($promo_code_stats)): ?>
                <div class="alert alert-info">No promo codes used within the selected filters for events in your venues.</div>
            <?php else: ?>
            ...
            <h3 class="section-subtitle">Top Events, Cities, & Venues (My Venues)</h3>
            <div class="row">
                <div class="col-md-4 mb-4">
                    <div class="metric-card">
                        <h5>Top 5 Events for My Venues by Revenue</h5>
                        <ul class="list-group list-group-flush">
                            <?php if (empty($top_events)): ?>
                                <li class="list-group-item">No events found for your venues.</li>
                            <?php else: ?>
                ...
                <div class="col-md-4 mb-4">
                    <div class="metric-card">
                        <h5>Top 5 Cities with My Venues by Revenue</h5>
                        <ul class="list-group list-group-flush">
                            <?php if (empty($top_cities)): ?>
                                <li class="list-group-item">No cities related to your venues found.</li>
                            <?php else: ?>
                ...
                <div class="col-md-4 mb-4">
                    <div class="metric-card">
                        <h5>Top 5 of My Venues by Revenue</h5>
                        <ul class="list-group list-group-flush">
                            <?php if (empty($top_venues)): ?>
                                <li class="list-group-item">No venues from your account found.</li>
                            <?php else: ?>
        ...
        if (filterCard && toggleFiltersBtn) {
            ...
            for (const [key, value] of urlParams.entries()) {
                if (key !== 'date_filter' && key !== 'custom_start_date' && key !== 'custom_end_date' && value) {
                    ...
                    if (urlParams.getAll(key).length > 0) { // Check if parameter exists as array
