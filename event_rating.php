    ...
    <div class="container" style="margin-top: 30px;">
        <?php if (!empty($success_message)): ?>
            <div class="success-message">
                ...
                <div class="mt-2">
                    <a href="<?=$event_info_page ;?>?id=<?php echo $event_id; ?>" class="btn btn-primary btn-sm">
...
<form method="POST" action="<?=$rate_event?>?event_id=<?php echo $event_id; ?>">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <textarea class="form-control" id="description" name="description" rows="3" placeholder="Share your thoughts..."></textarea>
                    </div>
                    
                    <div class="form-group mb-3">
                        <label for="hashtag" class="form-label"><strong>Hashtag (optional)</strong></label>
                        <input type="text" class="form-control" id="hashtag" name="hashtag" placeholder="#awesome #mustwatch">
                    </div>
                    
                    <div class="button-container">
                        <button type="submit" class="btn btn-submit">Submit Rating</button>
                        <a href="<?=$event_info_page ;?>?id=<?php echo $event_id; ?>" class="btn btn-outline-secondary">Cancel</a>
                    </div>
                </form>
            </div>
        <?php endif; ?>
        ...
        fetch(`<?=$search;?>?search=${encodeURIComponent(searchTerm)}`)
            ...
            .then(data => {
                if (data.length > 0) {
                    ...
                    data.forEach(item => {
                        html += `
                            <?=$event_info_page ;?>?id=${item.event_id}" class="search-result-item">
