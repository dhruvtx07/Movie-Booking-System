...
$totalReviewsStmt = $pdo->prepare("SELECT COUNT(*) FROM event_ratings WHERE created_by = ?");
...
$reviewsStmt = $pdo->prepare("
    ...
    LIMIT ? OFFSET ?
");
$reviewsStmt->bindValue(1, $userId, PDO::PARAM_INT);
$reviewsStmt->bindValue(2, $reviewsPerPage, PDO::PARAM_INT);
...
$reviewToEdit = null;
if ($editingReview && isset($_GET['rating_id'])) {
...
<!-- Top Navigation Bar -->
    ...
    <div class="container py-5 mt-5">
        ...
        <?php if ($editingReview && $reviewToEdit): ?>
            <div class="edit-review-container">
                <h2 class="mb-4">Edit Review</h2>
                
                <div class="review-container">
                    <h4 class="section-title">Editing review for <?= htmlspecialchars($reviewToEdit['event_name']) ?></h4>
                    
                    <form method="post" action="">
                        <input type="hidden" name="action" value="update_rating">
                        <input type="hidden" name="rating_id" value="<?= $reviewToEdit['rating_id'] ?>">
                        
                        <div class="mb-3">
                            <label class="form-label">Your Rating:</label>
                            <div class="rating-stars">
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <i class="fas fa-star <?= $i <= $reviewToEdit['user_gave'] ? 'star-filled' : 'star-empty' ?>" data-rating="<?= $i ?>"></i>
                                <?php endfor; ?>
                                <input type="hidden" name="rating" id="rating-value" value="<?= $reviewToEdit['user_gave'] ?>">
                            </div>
                        </div>
                                
                        <div class="mb-3">
                            <label for="rating_desc" class="form-label">Review:</label>
                            <textarea class="form-control" id="rating_desc" name="rating_desc" rows="3" required><?= htmlspecialchars($reviewToEdit['rating_desc']) ?></textarea>
                        </div>
                        
                        <div class="mb-3">
                            <label for="hashtag" class="form-label">Tags (optional):</label>
                            <input type="text" class="form-control" id="hashtag" name="hashtag" placeholder="#action #fun" value="<?= htmlspecialchars($reviewToEdit['hastag'] ?? '') ?>">
                        </div>
                        
                        <div class="action-buttons">
                            <button type="submit" class="btn btn-gradient-primary">Update Review</button>
                            <a href="<?=$my_reviews?>" class="btn btn-secondary">Cancel</a>
                        </div>
                    </form>
                </div>
            </div>
        <?php else: ?>
            <div class="reviews-page">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2>My Reviews</h2>
                    <div class="total-reviews">Total: <?= $totalReviews ?> reviews</div>
                </div>
                ...
                <?php else: ?>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="text-muted mt-2">
                                            ...
                                            <?php if ($rating['created_on']): ?>
                                                (updated <?= date('M j, Y g:i A', strtotime($rating['created_on'])) ?>)
                                            <?php endif; ?>
                                        </div>
                                        <div class="review-actions mt-3">
                                            <a href="<?=$edit_review?>?edit_review=1&rating_id=<?= $rating['rating_id'] ?>" class="btn btn-sm btn-primary">Edit</a>
                                            ...
                                            <a href="<?=$event_info_page?>?id=<?= $rating['event_id'] ?>" class="btn btn-sm btn-outline-secondary">View Event</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="d-flex justify-content-between mt-4">
                        ...
                        <?php if (($page * $reviewsPerPage) < $totalReviews): ?>
    ...
    <script>
        ...
        location.reload(true); // Force full reload from server
    }
